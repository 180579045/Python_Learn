...
sheet = book.sheet_by_index(14)                               # 根据sheet页名字获取sheet
sheetaa46 = book.sheet_by_index(0)                            # 默认用第一个sheet做替换
...
# 替换
for i in range(sheetaa46.nrows):
    if str(sheetaa46.row_values(i)[2]) in ref01:
        ret = ref01[str(sheetaa46.row_values(i)[2])]
        ws.write(i, 2, str(ret))
        print(ret)


for i in range(sheetaa46.nrows):
    if str(sheetaa46.row_values(i)[3]) in ref02:
        ret = ref02[str(sheetaa46.row_values(i)[3])]
